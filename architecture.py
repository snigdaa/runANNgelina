...
class model_2hl(nn.Module):
    ...
    def __init__(self, inp, h1, h2, out, dr1, dr2):
        ...
        self.dropout1   = nn.Dropout(p=dr1)
        self.dropout2   = nn.Dropout(p=dr2)
        self.ReLU      = nn.ReLU()
    ...
    # forward pass
    def forward(self, x):
        out = self.dropout1(self.LeakyReLU(self.fc1(x))) 
        out = self.dropout2(self.LeakyReLU(self.fc2(out)))
        out = self.fc3(out)         
        return out
##################################
...
class model_3hl(nn.Module):
    ...
    def __init__(self, inp, h1, h2, h3, out, dr1, dr2, dr3):
        ...
        self.dropout1   = nn.Dropout(p=dr1)
        self.dropout2   = nn.Dropout(p=dr2)
        self.dropout3   = nn.Dropout(p=dr3)
        self.ReLU      = nn.ReLU()
        self.LeakyReLU = nn.LeakyReLU()
    ...
    def forward(self, x):
        out = self.dropout1(self.LeakyReLU(self.fc1(x)))
        out = self.dropout2(self.LeakyReLU(self.fc2(out)))
        out = self.dropout3(self.LeakyReLU(self.fc3(out)))
        out = self.fc4(out)         
        return out
##################################
...
class model_4hl(nn.Module):
    ...
    def __init__(self, inp, h1, h2, h3, h4, out, dr1, dr2, dr3, dr4):
